...
- (void)layoutChildView
{
    ...
    if (self.numberOfPages > 0)
    {
        ...
        // Add missing pages.
        for (int pageIndex = (int)visibleRange.location; pageIndex < (NSInteger)NSMaxRange(visibleRange); ++pageIndex)
        {
...
- (void)displayChildPageAtIndex:(NSInteger)pageIndex {
    ...
    int oriX = (oldFrm.size.width*pageIndex) + (oldFrm.size.width - frame.size.width)/2;
    int oriY = (oldFrm.size.height-frame.size.height)/2;
...
- (NSRange)visiblePageRange {
    ...
    NSInteger firstVisiblePageIndex = calculateBoundary(currentVisiblePageIndex - 1, 0, self.numberOfPages - 1);
    NSInteger lastVisiblePageIndex  = calculateBoundary(currentVisiblePageIndex + 1, 0, self.numberOfPages - 1);
    
    NSRange range =  NSMakeRange(firstVisiblePageIndex, lastVisiblePageIndex - firstVisiblePageIndex + 1);
    
    NSLog(@"[%@]-->visible range:{%li,%li}",[self class],(long)firstVisiblePageIndex,(long)lastVisiblePageIndex);
